...
log_dir = "../data/logs/kahypar_logs"
output_file = f"{output_dir}combined_kahypar_logs.txt"
...
def parse_log_file(file_path):
    ...
    seed = re.search(r"seed:\s+(-?\d+)", content)
    partition_time = re.search(r"Partition time\s+=\s+([\d.]+)\s+s", content)
    hyperedge_cut = re.search(
        r"Hyperedge Cut\s+\(minimize\)\s+=\s+(\d+)", content
    )
    imbalance = re.search(r"Imbalance\s+=\s+([\d.]+)", content)

    # Extract partition sizes
    partition_sizes = re.findall(
        r"\|part\s+\d+\s+\|\s+=\s+(\d+)\s+w\(\s+\d+\s+\)\s+=\s+\d+", content
    )
    ...
    if command_line:
        ...
        for i in range(len(command_parts)):
            if command_parts[i] == "-p" and i + 1 < len(command_parts):
                preset = get_filename(
                    command_parts[i + 1]
                )  # Extract preset filename
            if i % 2 == 0 and i + 1 < len(command_parts):
                command_dict[command_parts[i]] = get_filename(
                    command_parts[i + 1]
                )
    ...
    return {
        ...
        "Partition Sizes": ", ".join(partition_sizes)
        if partition_sizes
        else "N/A",
