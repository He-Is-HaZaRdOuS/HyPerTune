...
input_file = 'input_data.csv'  # Replace with the path to your actual input file
...
# Assuming the columns are 'Matrix', 'Preset', and 'Average - Cut'
# Group by matrix and preset, and find the minimum cut for each matrix
min_cut_per_matrix = df.groupby('Matrix')['Average - Cut'].min()

# Now, for each matrix, find the preset that produces the minimum cut
matrix_preset_min_cut = pd.DataFrame(columns=['Matrix', 'Preset', 'Minimum Cut'])
...
for matrix in min_cut_per_matrix.index:
    ...
    min_cut_row = matrix_data[matrix_data['Average - Cut'] == min_cut_value]

    for _, row in min_cut_row.iterrows():
        matrix_preset_min_cut = matrix_preset_min_cut._append({
            'Matrix': matrix,
            'Preset': row['Preset'],
            'Minimum Cut': min_cut_value
        }, ignore_index=True)
